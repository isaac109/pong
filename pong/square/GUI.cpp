...
{
	determineOpponentMovement(key);
}

void GUI::determineOpponentMovement(char key)
{
	switch(key)
	{
	case 'u':
...
{
	determinePlayerMovement(key, i, j);
}

void GUI::determinePlayerMovement(int key, int, int)
{
	if(!data._isStart)
	{
		switch(key)
